...
export class TownsController extends Controller {
  ...
  public async createPosterSessionArea(
    ...
    if (!town.getPlayerBySessionToken(sessionToken)) {
      throw new InvalidParametersError('Invalid sessionToken specified');
    }

    if (requestBody.title === undefined) {
      throw new InvalidParametersError('Invalid requestBody: title must be included');
    }
    const posterSessionAreaCreated = town.addPosterSessionArea(requestBody);
    if (!posterSessionAreaCreated) {
      throw new InvalidParametersError('Invalid requestBody specified');
    }
  }
  ...
  public async getPosterAreaImageContents(
    ...
    const town = this._townsStore.getTownByID(townID);
    if (!town) {
      throw new InvalidParametersError('Invalid townID specified');
    }
    const townInterctable = town.getInteractable(posterSessionId);
    if (!townInterctable) {
      throw new InvalidParametersError('Invalid posterSessionId specified');
    }
    const sucess = town.getPlayerBySessionToken(sessionToken);
    if (!sucess) {
      throw new InvalidParametersError('Invalid sessionToken specified');
    }
    const townIsPosterSessionArea: boolean = isPosterSessionArea(townInterctable);
    if (!townIsPosterSessionArea) {
      throw new InvalidParametersError('Specified InteractableArea is not a PosterSessionArea');
    }

    const area = town.interactables.find(
      eachArea => eachArea.id === posterSessionId,
    ) as PosterSessionArea;
    return area.imageContents;
  }
  ...
  public async incrementPosterAreaStars(
    ...
    const townInterctable = town.getInteractable(posterSessionId) as PosterSessionArea;
    if (!townInterctable) {
      throw new InvalidParametersError('Invalid posterSessionId specified');
    }
    ...
    if (townInterctable.id !== posterSessionId) {
      throw new InvalidParametersError('Area not found found');
    }

    const townModel = townInterctable.toModel();
    townModel.stars += 1;
    townInterctable.updateModel(townModel);

    return townModel.stars;
  }
