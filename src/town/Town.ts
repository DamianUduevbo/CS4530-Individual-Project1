...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('chatMessage', (message: ChatMessage) => {
      if (newPlayer.location.interactableID === message.interactableId) {
        this._connectedSockets.forEach(eachSock => eachSock.emit('chatMessage', message));
      }
    });
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      try {
        const area = this.getInteractable(update.id);
        ...
        if (isPosterSessionArea(update)) {
          const updatedPosterSessionArea = update as PosterSessionArea;
          (area as PosterSessionArea).updateModel(updatedPosterSessionArea);
          newPlayer.townEmitter.emit('interactableUpdate', update);
        } else if (isViewingArea(update)) {
          const updatedViewingArea = update as ViewingArea;
          (area as ViewingArea).updateModel(updatedViewingArea);
          newPlayer.townEmitter.emit('interactableUpdate', update);
        }
      } catch {
        /* EMPTY BLOCK */
  ...
  public addPosterSessionArea(posterSessionArea: PosterSessionAreaModel): boolean {
    ...
    if (!area || !posterSessionArea.imageContents || area.imageContents) {
      return false;
    }
